      ...
      <div class="zc-modal-body psm-body">
        ...
        <template v-if="activeTab === 'general'">
          ...
          <div class="psm-field">
            ...
            <input class="zc-input" v-model="formName" placeholder="Project name…" />
          </div>
          ...
          <div class="psm-field">
            ...
            <div class="psm-logo-row">
              <div class="psm-logo-thumb" :style="{ background: formColor }">
                <img
                  v-if="logoPreview"
                  :src="/^[A-Za-z]:/.test(logoPreview) || logoPreview.startsWith('/') ? `file:///${logoPreview.replaceAll('\\', '/')}` : `zntt-userdata://${logoPreview}`"
        ...
        <template v-if="activeTab === 'columns'">
          ...
          <div class="psm-col-list">
            ...
            >
              ...
              <!-- Done radio -->
              <label class="psm-done-lbl" @click.prevent="setDoneCol(col)">
              ...
              <button
                ...
                :title="col.tasks?.length > 0
                  ? `Cannot delete — ${col.tasks.length} task(s) inside. Move them first.`
                  : 'Delete column'"
                @click="deleteCol(col)"
              >
...
const PROJ_COLORS = [
  '#7c6af7', '#3ecfcf', '#e05c5c', '#4caf82',
  '#e0a050', '#5c8ee0', '#c050e0', '#e0608a',
]
...
const formInitials = computed(() =>
  (formName.value || 'P').slice(0, 2).toUpperCase()
)
...
onMounted(async () => {
  ...
  localCols.value = props.columns.map(c => ({ ...c, tasks: c.tasks || [] }))
})
...
async function saveGeneral() {
  ...
  try {
    ...
    if (catName) {
      let cat = allCategories.value.find(
        c => c.name.toLowerCase() === catName.toLowerCase()
      )
      if (!cat) cat = await api.createCategory(catName)
      changes.category_id = cat.id
    } else {
      changes.category_id = null
    }

    // Logo: new file → copy; removed → clear
    if (newLogoPath.value) {
      await api.saveProjectLogo(props.project.id, newLogoPath.value)
      newLogoPath.value = null
    } else if (!logoPreview.value && props.project.logo_path) {
      changes.logo_path = null
    }

    await api.updateProject(props.project.id, changes)
    emit('updated')
    emit('close')
  } finally {
    saving.value = false
...
async function persistColOrder() {
  const ids = localCols.value.map(c => c.id)
  await api.reorderColumns(props.project.id, ids)
  emit('columnsChanged')
}
...
async function setDoneCol(col) {
  // Radio: unset all others first
  for (const c of localCols.value) {
    if (c.is_done_column && c.id !== col.id) {
      c.is_done_column = 0
      await api.updateColumn(c.id, { is_done_column: 0 })
    }
  }
  col.is_done_column = col.is_done_column ? 0 : 1
  await api.updateColumn(col.id, { is_done_column: col.is_done_column })
  emit('columnsChanged')
}
...
async function deleteCol(col) {
  ...
  localCols.value = localCols.value.filter(c => c.id !== col.id)
  emit('columnsChanged')
}
